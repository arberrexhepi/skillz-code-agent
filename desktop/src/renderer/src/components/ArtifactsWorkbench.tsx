...
import type { ArtifactLibrary, ArtifactRecord, ArtifactRuntime, PrebuiltArtifact } from '../../../shared/artifacts';
...
import { ArtifactDockerStorage } from './ArtifactDockerStorage';
...
export function ArtifactsWorkbench({ visible, sourceRoot, onClose }: { visible: boolean; sourceRoot: string; onClose: () => void }): React.JSX.Element {
  ...
  const [prebuilts, setPrebuilts] = useState<PrebuiltArtifact[]>([]);
  const [preset, setPreset] = useState<PrebuiltArtifact>();
  ...
  useEffect(() => { void Promise.all([window.workbench.artifacts.library(), window.workbench.artifacts.prebuilts()]).then(([library, prebuilts]) => { setLibrary(library); setPrebuilts(prebuilts); }).catch((error) => setError(clean(error))); return window.workbench.artifacts.onEvent((event) => { if (event.type === 'runtime') setRuntimes((current) => ({ ...current, [event.runtime.id]: event.runtime })); }); }, []);
  async function configure() { setBusy(true); setError(''); try { const next = await window.workbench.artifacts.chooseFolder(); if (next) { setLibrary(next); setOpen([]); setActive(''); setRuntimes({}); setPreset(undefined); setCreating(true); } } catch (error) { setError(clean(error)); } finally { setBusy(false); } }
  function openArtifact(id: string) { setPreset(undefined); setOpen((current) => current.includes(id) ? current : [...current, id]); setActive(id); setCreating(false); setPreset(undefined); }
  ...
  async function installPreset() {
    if (!preset || busy || (preset.requiresWriteAccess && !access.directories.some(directory => directory.access === 'write')) || (preset.requiresProcessProxy && !access.directories.some(directory => directory.allowProcessProxy))) return;
    setBusy(true); setError('');
    try {
      const record = await window.workbench.artifacts.installPrebuilt(preset.id, access, { ...creationRuntime, backendScript: creationRuntime.backendScript as 'main.py' | 'main_v2.py' | 'live_test_loop.py' });
      setLibrary(current => ({ ...current, artifacts: [...current.artifacts, record] }));
      openArtifact(record.id); setAccess(emptyAccess);
    } catch (error) { setError(clean(error)); } finally { setBusy(false); }
  }
  ...
  return <section className="artifacts-workbench" hidden={!visible} aria-label="Artifacts workbench">
    ...
    <div className="artifacts-layout">
      <aside className="artifacts-library"><header><strong>LIBRARY</strong><button aria-label="New artifact" disabled={!library.root || busy} onClick={() => { setPreset(undefined); setCreating(true); }}>＋</button></header><p className="artifacts-location" title={library.root}>{library.root || 'Choose where your artifacts live.'}</p><button disabled={busy} onClick={() => void configure()}>{library.root ? 'Change folder' : 'Choose artifacts folder'}</button>{library.root && !library.artifacts.length && <p className="muted">Your first artifact starts with an idea.</p>}<nav aria-label="Artifact library">{library.artifacts.map((record) => <button className={active === record.id && !creating && !preset ? 'active' : ''} key={record.id} onClick={() => openArtifact(record.id)}><strong>{record.title}</strong><span>{runtimes[record.id]?.status || 'Ready to open'}</span></button>)}</nav><div className="artifacts-library-footer"><button type="button" onClick={() => setStorageOpen(true)}>Docker storage</button><p className="artifacts-library-note">Each artifact has its own Git history, linked as a submodule in this library.</p></div></aside>
      <div className="artifacts-content"><div className="artifact-tabs" role="tablist" aria-label="Open artifacts">{open.map((id) => <div key={id} className={active === id && !creating && !preset ? 'active' : ''}><button role="tab" aria-selected={active === id && !creating && !preset} onClick={() => { setActive(id); setCreating(false); setPreset(undefined); }}>{library.artifacts.find((item) => item.id === id)?.title}</button><button aria-label={`Close ${library.artifacts.find((item) => item.id === id)?.title}`} onClick={() => void closeTab(id)}>×</button></div>)}</div>
        {(!library.root || (!open.length && !creating && !preset)) && <div className="artifact-empty"><span>◇</span><h2>What would you like to make?</h2><p>A database schema map. A code relationship explorer. A dashboard for your project.</p>{library.root ? <><button className="primary-button" onClick={() => { setPreset(undefined); setCreating(true); }}>Create an artifact</button>{prebuilts.map(item => <button key={item.id} onClick={() => { setAccess(emptyAccess); setPreset(item); setCreating(false); }}>Install {item.title}</button>)}</> : <><p>Choose an empty folder. skillz will initialize a Git repository for your library.</p><ArtifactCapabilities selection={creationRuntime} firstTime onReadyChange={setCapabilitiesReady} onBusyChange={setSetupBusy} /><button className="primary-button" disabled={busy} onClick={() => void configure()}>Choose artifacts folder</button></>}</div>}
        {preset && library.root && <form className="artifact-create artifact-prebuilt-install" onSubmit={event => { event.preventDefault(); void installPreset(); }}><span className="eyebrow">READY-MADE ARTIFACT</span><h2>{preset.title}</h2><p>{preset.description}</p><p>Add one or more repository folders below.{preset.requiresWriteAccess && <> Enable <strong>Allow changes</strong> for repositories this artifact should modify.</>}{preset.requiresProcessProxy && <> Enable <strong>Allow Process Proxy</strong> for repositories whose scripts it may run.</>}</p><ArtifactAccessFields value={access} onChange={setAccess} sourceRoot={sourceRoot} disabled={busy} /><div className="artifact-form-actions"><button type="button" onClick={() => { setPreset(undefined); setCreating(true); }}>Back</button><button className="primary-button" disabled={busy || Boolean(preset.requiresWriteAccess && !access.directories.some(directory => directory.access === 'write')) || Boolean(preset.requiresProcessProxy && !access.directories.some(directory => directory.allowProcessProxy))}>{busy ? 'Installing…' : `Install ${preset.title}`}</button></div></form>}
        {creating && !preset && library.root && <form className="artifact-create" onSubmit={(event) => { event.preventDefault(); void create(); }}><span className="eyebrow">NEW ARTIFACT</span><h2>Start with an idea.</h2>{prebuilts.length > 0 && <section className="artifact-prebuilt-callout"><strong>Prefer something ready to use?</strong><p>Install a maintained artifact, then choose exactly which folders it can access.</p>{prebuilts.map(item => <button type="button" key={item.id} onClick={() => { setAccess(emptyAccess); setPreset(item); setCreating(false); }}>Install {item.title}</button>)}</section>}<ArtifactCapabilities selection={creationRuntime} firstTime={!library.artifacts.length} onReadyChange={setCapabilitiesReady} onBusyChange={setSetupBusy} /><label>Name<input required maxLength={120} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Database schema explorer" /></label><label>What should the agent build?<textarea required maxLength={20000} rows={6} value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Visualize our tables and their relationships, with filters and clickable details…" /></label><fieldset disabled={!sourceRoot}><legend>Share source workspace context</legend><p>{sourceRoot || 'Open a workspace to share its context files.'}</p><label><input type="checkbox" checked={facts} onChange={(event) => setFacts(event.target.checked)} /> Repo facts</label><label><input type="checkbox" checked={memory} onChange={(event) => setMemory(event.target.checked)} /> Memory observability</label></fieldset><ArtifactAccessFields value={access} onChange={setAccess} sourceRoot={sourceRoot} disabled={busy} /><fieldset className="artifact-runtime-picker" disabled={busy}><legend>Agent runtime</legend><RuntimeSelectionFields value={creationRuntime} onChange={setCreationRuntime} options={sourceAgent.state.runtimeOptions} disabled={busy || setupBusy} onReadyChange={setRuntimeReady} /></fieldset><button className="primary-button" disabled={busy || setupBusy || !capabilitiesReady || !runtimeReady || !title.trim() || !prompt.trim()}>{busy ? 'Creating…' : 'Create & ask agent'}</button></form>}
        {open.map((id) => { const record = library.artifacts.find((item) => item.id === id); return record && <ArtifactSession key={id} record={record} sourceRoot={sourceRoot} active={visible && active === id && !creating && !preset} runtime={runtimes[id]} initialRuntime={record.runtime || sourceAgent.runtime} request={requested.includes(id)} onRequested={() => setRequested((current) => current.filter((value) => value !== id))} />; })}
      </div>
    </div>
    {storageOpen && <ArtifactDockerStorage onClose={() => setStorageOpen(false)} />}
  </section>;
}
