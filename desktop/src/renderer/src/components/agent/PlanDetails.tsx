...
export function PlanDetails({ plan }: { plan: PlannerPlan }): React.JSX.Element {
  ...
  return <div className="plan-details">
    ...
    <ol className="plan-goals">{goals.map((goal, index) => <li key={goal.goal_id || index}>
      <h4><span aria-hidden="true">{index + 1}</span>{goal.title || goal.goal || goal.goal_id}</h4>
      {goal.goal && goal.goal !== goal.title && <p>{goal.goal}</p>}
      {goal.reason && <p><strong>Why:</strong> {goal.reason}</p>}
      <dl className="plan-goal-meta">
        {goal.estimated_scope && <><dt>Scope</dt><dd>{goal.estimated_scope}</dd></>}
        {!!goal.depends_on?.length && <><dt>Depends on</dt><dd>{goal.depends_on.map(goalLabel).join(', ')}</dd></>}
...
function TextSection({ title, text }: { title: string; text?: string }): React.JSX.Element | null {
  return text ? <section><h3>{title}</h3><p>{text}</p></section> : null;
}
